...
C8master = outerjoin(Mtable,Etable,'MergeKeys',true); %join on time collumn, hence rounding to nearest 10 s
%writetable(C8master,'C8Stress.csv')

%make list of half cycle indices
[l hCycIndex] = findgroups(C8master.halfCycle);
if hCycIndex(1)==0
    hCycIndex = hCycIndex(2:end);
end
%build subtables from each half cycle and just charge cycles
for i = hCycIndex'
    thisCycle = find(C8master.halfCycle==i);
    thisArr = table2array(C8master(thisCycle,:));
...
[l fCycIndex] = findgroups(C8master.cycleNumber);
if fCycIndex(1)==0
    fCycIndex = fCycIndex(2:end);
end
%build subtables from each FULL cycle and rezero time
for i = fCycIndex'
    thisCycle = find(C8master.cycleNumber==i);
    startVals = C8master(thisCycle(1),:);
    Cycles{i} = C8master(thisCycle,:);
    %Cycles{i}.time_s = Cycles{i}.time_s - startVals.time_s;
end

figure(1)
plot(C8master.time_s/3600,C8master.stress_MPa);
xlabel('Time (Hrs)');
hold on;
plot(C8master.time_s/3600,C8master.x_I__mA)
plot(C8master.time_s/3600,C8master.Ewe_V)
title(projdir)
...
%writetable(chrono,'C8Chronology.csv')
