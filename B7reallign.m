...
B7master = outerjoin(Mtable,Etable,'MergeKeys',true); %join on time collumn, hence rounding to nearest 10 s
% UNCOMMENT THIS TO MAKE A CSV FILE
%writetable(B7master,fullfile(projdir,'Baseline4Stress.csv'))

%make list of half cycle indices
[l hCycIndex] = findgroups(B7master.halfCycle);
if hCycIndex(1)==0  
    hCycIndex = hCycIndex(2:end);
end
%build subtables from each half cycle and just charge cycles
for i = hCycIndex'
    thisCycle = find(B7master.halfCycle==i);
    startVals = B7master(thisCycle(1),:);
    halfCycles{i} = B7master(thisCycle,:);
    %halfCycles{i}.time_s = halfCycles{i}.time_s - startVals.time_s;
    if startVals.ox_red == 0
        chargeCycs{i} = table2array(B7master(thisCycle,:));
    elseif startVals.ox_red == 1
        dischargeCycs{i} = table2array(B7master(thisCycle,:));
    end
end
...
[l fCycIndex] = findgroups(B7master.cycleNumber);
if fCycIndex(1)==0
    fCycIndex = fCycIndex(2:end);
end
%build subtables from each FULL cycle and rezero time
for i = fCycIndex'
    thisCycle = find(B7master.cycleNumber==i);
    startVals = B7master(thisCycle(1),:);
    Cycles{i} = B7master(thisCycle,:);
    %Cycles{i}.time_s = Cycles{i}.time_s - startVals.time_s;
end

figure(1)
plot(B7master.time_s/3600,B7master.stress_MPa);
xlabel('Time (Hrs)'); 
hold on;
plot(B7master.time_s/3600,B7master.x_I__mA)
plot(B7master.time_s/3600,B7master.Ewe_V)
title(projdir)
...
% UNCOMMENT THIS TO MAKE A CSV FILE
%writetable(chrono,'B7Chronology.csv')
