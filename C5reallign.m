...
C5master = outerjoin(Mtable,Etable,'MergeKeys',true); %join on time collumn, hence rounding to nearest 10 s
C5master = C5master(~isnan(C5master.stress_MPa),:);%get rid of empty rows due to MOSS downsampling
%writetable(C5master,'C5Stress.csv')

%make list of half cycle indices
[l hCycIndex] = findgroups(C5master.halfCycle);
if hCycIndex(1)==0
    hCycIndex = hCycIndex(2:end);
end
%build subtables from each half cycle and just charge cycles
for i = hCycIndex'
    thisCycle = find(C5master.halfCycle==i);
    thisArr = table2array(C5master(thisCycle,:));
...
[l fCycIndex] = findgroups(C5master.cycleNumber);
if fCycIndex(1)==0
    fCycIndex = fCycIndex(2:end);
end
%build subtables from each FULL cycle and rezero time
for i = fCycIndex'
    thisCycle = find(C5master.cycleNumber==i);
    startVals = C5master(thisCycle(1),:);
    Cycles{i} = C5master(thisCycle,:);
    %Cycles{i}.time_s = Cycles{i}.time_s - startVals.time_s;
end

figure(1)
plot(C5master.time_s/3600,C5master.stress_MPa);
xlabel('Time (Hrs)');
hold on;
plot(C5master.time_s/3600,C5master.x_I__mA)
plot(C5master.time_s/3600,C5master.Ewe_V)
title(projdir)
...
%writetable(chrono,'C5Chronology.csv')
