...
hs = 501e-6;%microns current substrate 
...
P9master = outerjoin(Mtable,Etable,'MergeKeys',true); %join on time collumn, hence rounding to nearest 10 s
% UNCOMMENT THIS TO MAKE A CSV FILE
%writetable(P9master,fullfile(projdir,'Pattern9Stress.csv'))

%make list of half cycle indices
[l hCycIndex] = findgroups(P9master.halfCycle);
if hCycIndex(1)==0
    hCycIndex = hCycIndex(2:end);
end
%build subtables from each half cycle and just charge cycles
for i = hCycIndex'
    thisCycle = find(P9master.halfCycle==i);
    startVals = P9master(thisCycle(1),:);
    halfCycles{i} = P9master(thisCycle,:);
    %halfCycles{i}.time_s = halfCycles{i}.time_s - startVals.time_s;
    if startVals.ox_red == 0
        chargeCycs{i} = table2array(P9master(thisCycle,:));
    elseif startVals.ox_red == 1
        dischargeCycs{i} = table2array(P9master(thisCycle,:));
    end
end
...
[l fCycIndex] = findgroups(P9master.cycleNumber);
if fCycIndex(1)==0
    fCycIndex = fCycIndex(2:end);
end
%build subtables from each FULL cycle and rezero time
for i = fCycIndex'
    thisCycle = find(P9master.cycleNumber==i);
    startVals = P9master(thisCycle(1),:);
    Cycles{i} = P9master(thisCycle,:);
    %Cycles{i}.time_s = Cycles{i}.time_s - startVals.time_s;
end


figure(1)
plot(P9master.time_s/3600,P9master.stress_MPa);
xlabel('Time (Hrs)');
hold on;
plot(P9master.time_s/3600,P9master.x_I__mA)
plot(P9master.time_s/3600,P9master.Ewe_V)
title(projdir)
...
% UNCOMMENT THIS TO MAKE A CSV FILE
%writetable(chrono,'Pattern9Chronology.csv')
...
hold off
grid on
legend('Charge Stress Change St','Discharge Stress Change End','Irreversible Stress',...
    'Charge Capacity Start','Discharge Capacity End','Irreversible Capacity')
xlabel('Cycle Number')
ylabel('Capacity mAh/g')
