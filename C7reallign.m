...
C7master = outerjoin(Mtable,Etable,'MergeKeys',true); %join on time collumn, hence rounding to nearest 10 s
C7master = C7master(~isnan(C7master.stress_MPa),:);%get rid of empty rows due to MOSS downsampling
%writetable(C7master,'C7Stress.csv')

%make list of half cycle indices
[l hCycIndex] = findgroups(C7master.halfCycle);
if hCycIndex(1)==0
    hCycIndex = hCycIndex(2:end);
end
%build subtables from each half cycle and just charge cycles
for i = hCycIndex'
    thisCycle = find(C7master.halfCycle==i);
    thisArr = table2array(C7master(thisCycle,:));
...
[l fCycIndex] = findgroups(C7master.cycleNumber);
if fCycIndex(1)==0
    fCycIndex = fCycIndex(2:end);
end
%build subtables from each FULL cycle and rezero time
for i = fCycIndex'
    thisCycle = find(C7master.cycleNumber==i);
    startVals = C7master(thisCycle(1),:);
    Cycles{i} = C7master(thisCycle,:);
    %Cycles{i}.time_s = Cycles{i}.time_s - startVals.time_s;
end

figure(1)
plot(C7master.time_s/3600,C7master.stress_MPa);
xlabel('Time (Hrs)');
hold on;
plot(C7master.time_s/3600,C7master.x_I__mA)
plot(C7master.time_s/3600,C7master.Ewe_V)
title(projdir)
...
%writetable(chrono,'C7Chronology.csv')
