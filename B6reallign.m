...
for k = 1:M
    ...
    %fix because reasons
    if k==1
        diff = MList{k}(22738,2)-MList{k}(22737,2);
        MList{k}(22738:end,2) =  MList{k}(22738:end,2)-diff;
    end
...
B6master = outerjoin(Mtable,Etable,'MergeKeys',true); %join on time collumn, hence rounding to nearest 10 s
% UNCOMMENT THIS TO MAKE A CSV FILE
%writetable(B6master,fullfile(projdir,'Baseline4Stress.csv'))

%make list of half cycle indices
[l hCycIndex] = findgroups(B6master.halfCycle);
if hCycIndex(1)==0  
    hCycIndex = hCycIndex(2:end);
end
%build subtables from each half cycle and just charge cycles
for i = hCycIndex'
    thisCycle = find(B6master.halfCycle==i);
    startVals = B6master(thisCycle(1),:);
    halfCycles{i} = B6master(thisCycle,:);
    %halfCycles{i}.time_s = halfCycles{i}.time_s - startVals.time_s;
    if startVals.ox_red == 0
        chargeCycs{i} = table2array(B6master(thisCycle,:));
    elseif startVals.ox_red == 1
        dischargeCycs{i} = table2array(B6master(thisCycle,:));
    end
end
...
[l fCycIndex] = findgroups(B6master.cycleNumber);
if fCycIndex(1)==0
    fCycIndex = fCycIndex(2:end);
end
%build subtables from each FULL cycle and rezero time
for i = fCycIndex'
    thisCycle = find(B6master.cycleNumber==i);
    startVals = B6master(thisCycle(1),:);
    Cycles{i} = B6master(thisCycle,:);
    %Cycles{i}.time_s = Cycles{i}.time_s - startVals.time_s;
end

figure(1)
plot(B6master.time_s/3600,B6master.stress_MPa);
xlabel('Time (Hrs)'); 
hold on;
plot(B6master.time_s/3600,B6master.x_I__mA)
plot(B6master.time_s/3600,B6master.Ewe_V)
title(projdir)
